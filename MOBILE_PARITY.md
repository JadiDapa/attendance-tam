...
| 4 | Request account | `/request-account` | form, success view, and duplicate-email error verified end-to-end (real submit against dev DB) | same path | **Built & verified** | — | New server action `requestAccount` in `account-request.action.ts` (web calls it directly instead of the mobile app's `POST /api/account-requests` fetch). Added `/request-account` to `proxy.ts`'s public-route matcher (was redirecting to `/login` otherwise). Login page's "contact your administrator" text now links here, matching mobile's "Ajukan pembuatan akun". |
...
| 14 | Attendance capture | `/absen` | not-enrolled gate verified end-to-end (real account state); office-not-configured gate, and the live camera+GPS+submit chain, built but not screenshot-verified — face-recognition microservice is unreachable in this dev environment, so enrollment (and therefore anything past the not-enrolled gate) can't be exercised here | `attendance-capture` | **Built**, partially verified | — | Full-page adaptation of `AttendanceDialog`'s camera+GPS+auto-submit logic (not a rewrite) — extracted the shared GPS-reading function into `lib/geo.ts` (`readAccuratePosition`) so both components use one implementation. `AttendanceDialog` itself refactored to use it too, no behavior change. |
| 15 | Face enrollment | `/verifikasi-wajah` | counter states + capture flow verified end-to-end (real submit surfaced the face-recognition service's own error correctly); reset-confirm dialog built but not screenshot-verified | `face-enrollment` | **Built**, mostly verified | — | Camera stays live and each capture posts immediately to `enrollFace` (no preview step), matching the RN source. Reset confirmation uses `AlertDialog` instead of RN `Alert.alert`. |
