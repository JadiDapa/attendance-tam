...
export type SelfAttendanceSearchParams = {
  start?: string;
  end?: string;
  view?: string;
  status?: string;
...
/**
 * Dashboard absensi diri sendiri — dipakai karyawan di `/dashboard` maupun
 * admin/supervisor/manager di `/admin/absensi`, `/supervisor/absensi`,
 * `/manager/absensi` saat mereka absen untuk dirinya sendiri. Satu komponen
 * yang sama, cuma `basePath` & `title` yang berbeda per role supaya link
 * (paginasi rentang tanggal, dll) tetap mengarah ke halaman yang benar.
 */
export default async function SelfAttendanceDashboard({
  user,
  searchParams,
  basePath,
  title,
  ...
  user: User;
  searchParams: SelfAttendanceSearchParams;
  basePath: string;
  title: string;
}) {
  ...
  // daftar riwayat mobile selalu menunjukkan absensi terbaru.
